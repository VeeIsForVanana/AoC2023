...
class Line:
    ...
    def findMaxValuesPerColor(self) -> list[int]:
        valuesDict: dict[Color: list[int]] = {Color.RED: [], Color.GREEN: [], Color.BLUE: []}
        
        # takes an dict[Color: int] and adds the value for each color to the corresponding list in the dict
        def buildColorList(acc: dict[Color: list[int]] , elem: dict[Color: int]):
            for key in elem.keys():
                acc[key].append(elem[key])
            return acc
        
        valuesDict = reduce(buildColorList, self.rounds, valuesDict)
        return list(map(lambda lst: max(lst), valuesDict.values()))
...
def main(filename: str) -> int:
    with open(filename) as file:
        ...
        while((line := file.readline()) != ""):
            print(line) 
            print(total := total + prod(Line(line).parseLine().findMaxValuesPerColor()))
        return total
